...
- When the user explicitly asks to upload, push, or save the finished article to a WeChat Official Account draft box, read [references/wechat-publishing.md](references/wechat-publishing.md) and then hand off to the separately installed `$baoyu-post-to-wechat` skill from the controlled fork recorded in [integrations/baoyu-post-to-wechat.lock](integrations/baoyu-post-to-wechat.lock).
...
- Invoke the locally installed publisher snapshot. Never fetch, install, or upgrade publisher code during a publishing request.
